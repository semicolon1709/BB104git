...
def forum_crawler(page_num, forum_name):
    ...
    for article in articles:
        ...
        try:
            ...
            article_dict['category'] = title.split("[")[1].split("]")[0]
    ...
    if len(article_list) != 0:
        ...
        db.ptt.insert_many(article_list)
...
def content_crawler(title, raw_date, article_dict, article):
    ...
    article_dict['time'] = soup_content.select("div.article-metaline")[2].\
                               select_one("span.article-meta-value").text.strip().split(" ")[3][0:5]
    # 如果是回復的文章，將引用本文內容的部分去除
    if title.startswith("Re"):
        article_dict['title'] = "Re: " + title.split("] ")[1]
        article_dict['content'] = soup_content.select_one("div#main-content").text.split(content_spliter)[1].\
        ...
        try:
            ...
            for quote in quote_list:
                article_dict['content'] = article_dict['content'].replace(quote, "")
            quote_list = [quote.text for quote in
    ...
    article_dict['category'] = soup_content.select("div.article-metaline")[1].\
        select_one("span.article-meta-value").text.split(']')[0].split('[')[1].strip()
    ...
    try:
        ...
        pass
    # 將文章內容以結巴斷詞
    article_dict["content"] = jieba_cut.cut(article_dict["content"])
    print(article_dict)
    return article_dict
